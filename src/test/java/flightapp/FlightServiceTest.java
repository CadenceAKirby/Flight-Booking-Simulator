...
public class FlightServiceTest {
  ...
  /**
   * We have a special "hidden" command that implements a barrier between all sessions; no
   * session may proceed until all sessions have entered the barrier.  This is useful for,
   * for example, designating one session to do the setup (eg, creating an account and
    ...
    public String call() {
      ...
      for (String cmd : cmds) {
        if (cmd.equals(BARRIER_COMMAND) && barrier != null) {
          // We have a special "hidden" command that implements a barrier across all sessions; no
          // session may proceed until all sessions have entered the barrier.  This is useful for,
          // for example, designating one session to do the setup (eg, creating an account and
          // booking a reservation) and then having all sessions attempt to do the test
          // concurrently (eg, paying for the reservation).
